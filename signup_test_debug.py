...
try:
    existing = User.objects.filter(username='test_debug_user').first()
    if existing:
        existing.delete()
...
try:
    existing = User.objects.filter(username='testdebug').first()
    if existing:
        existing.delete()
        print("✓ Ancien utilisateur testdebug supprimé")
except:
    pass

try:
    existing = User.objects.filter(email='testdebug@example.com').first()
    if existing:
        existing.delete()
        print("✓ Ancien utilisateur avec email testdebug@example.com supprimé")
except:
    pass

try:
    existing = User.objects.filter(username='Leley').first()
    if existing:
        existing.delete()
        print("✓ Ancien utilisateur Leley supprimé")
except:
    pass

try:
    existing = User.objects.filter(email='leley@example.com').first()
    if existing:
        existing.delete()
        print("✓ Ancien utilisateur avec email leley@example.com supprimé")
except:
    pass

try:
    existing = User.objects.filter(username='Gaboom').first()
    if existing:
        existing.delete()
        print("✓ Ancien utilisateur Gaboom supprimé")
except:
    pass

try:
    existing = User.objects.filter(email='gaboom@example.com').first()
    if existing:
        existing.delete()
        print("✓ Ancien utilisateur avec email gaboom@example.com supprimé")
except:
    pass
...
client = Client()
data = {
    'username': 'Leley',  # Utiliser un nom d'utilisateur comme l'utilisateur a demandé
    'email': 'leley@example.com',
    'phone': '+50912345678',
    'borlette_name': 'Leley Borlette Test',
...
try:
    ...
    if result.get('success'):
        ...
        user = User.objects.filter(username='test_debug_user').first()
        if user:
